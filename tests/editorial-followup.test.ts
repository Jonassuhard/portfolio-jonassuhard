...
test("Cool Bank distingue la lecture recente des essais historiques", () => {
  ...
  assert.equal(item?.checkedAt, "2026-09-23");
  assert.match(item?.note ?? "", /sans recette complète/);
  ...
  assert.equal(profile.citable_facts.cool_bank_la_herse_versions.checked_at, "2026-09-23");
});
